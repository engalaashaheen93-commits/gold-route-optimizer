...
T = {
    ...
    "robust_hdr":      {"en": "Decision Robustness (Sensitivity Analysis)",
                        "ar": "متانة القرار (تحليل الحساسية)"},
    "priority_hdr":    {"en": "Shipment Priority (optional)",
                        "ar": "أولوية الشحنة (اختياري)"},
    "priority_label":  {"en": "Describe what matters most for this shipment",
                        "ar": "صِف ما يهمّك أكثر في هذه الشحنة"},
    "priority_ph":     {"en": "e.g. urgent delivery for a key client, cost is secondary, and I'm concerned about regional tensions",
                        "ar": "مثال: شحنة عاجلة لعميل مهم، التكلفة ثانوية، وأخشى التوترات في المنطقة"},
    "priority_help":   {"en": "Leave empty to use the default weights. If filled, an AI model converts your priority into criterion weights that drive the ranking.",
                        "ar": "اتركه فارغاً لاستخدام الأوزان الافتراضية. وإن ملأته، يحوّل نموذج ذكاء اصطناعي أولويتك إلى أوزان معايير تُستخدم في الترتيب."},
    "weights_hdr":     {"en": "Weights Derived From Your Priority",
                        "ar": "الأوزان المشتقّة من أولويتك"},
    "weights_default": {"en": "Default",  "ar": "الافتراضي"},
    "weights_derived": {"en": "Derived",  "ar": "المشتقّ"},
    "criterion_col":   {"en": "Criterion", "ar": "المعيار"},
    "weights_note":    {"en": "These weights replaced the defaults in the TOPSIS ranking above. Clear the priority box to revert.",
                        "ar": "حلّت هذه الأوزان محل الافتراضية في ترتيب TOPSIS أعلاه. أفرغ خانة الأولوية للعودة إليها."},
    "weights_invalid": {"en": "The priority could not be converted into valid weights, so the default weights were used.",
                        "ar": "تعذّر تحويل الأولوية إلى أوزان صالحة، فاستُخدمت الأوزان الافتراضية."},
    "robust_line":     {"en": "The recommended route stayed first in {wins} of {total} perturbed scenarios.",
                        "ar": "بقي المسار الموصى به في المرتبة الأولى في {wins} من أصل {total} سيناريو مُعدَّل."},
    "robust_high":     {"en": "HIGH — the recommendation is stable.",
                        "ar": "متانة عالية — التوصية مستقرة."},
    "robust_moderate": {"en": "MODERATE — the recommendation shifts under some conditions.",
                        "ar": "متانة متوسطة — تتبدّل التوصية في بعض الظروف."},
    "robust_fragile":  {"en": "FRAGILE — the recommendation is sensitive to small changes.",
                        "ar": "متانة ضعيفة — التوصية حسّاسة لتغيّرات صغيرة."},
    "robust_weights":  {"en": "Weight changes",      "ar": "تغيّر الأوزان"},
    "robust_cost":     {"en": "Cost changes",        "ar": "تغيّر التكلفة"},
    "robust_time":     {"en": "Time changes",        "ar": "تغيّر الزمن"},
    "robust_flips":    {"en": "Scenarios that changed the winner ({n})",
                        "ar": "السيناريوهات التي غيّرت المسار الأول ({n})"},
    "margin_line":     {"en": "Score gap over runner-up: {gap} (cost {cost}, time {time}h)",
                        "ar": "فارق الدرجة عن المسار التالي: {gap} (تكلفة {cost}، زمن {time} ساعة)"},
    "margin_thin":     {"en": "the gap is narrow, so the top routes are near-equivalent",
                        "ar": "الفارق ضيّق، فالمساران الأولان شبه متكافئين"},
    "breakeven_line":  {"en": "it keeps first place until its own cost rises by ~{pct}%",
                        "ar": "يبقى أولاً حتى ترتفع تكلفته بنحو {pct}%"},
    "breakeven_none":  {"en": "it holds first place across the whole tested cost range",
                        "ar": "يبقى أولاً على امتداد نطاق التكلفة المختبَر"},
    "robust_note":     {"en": "Perturbations tested: each criterion weight ±10% and ±20%, and total cost / transit time ±10% and ±20% across all routes.",
                        "ar": "السيناريوهات المختبَرة: وزن كل معيار ±10% و±20%، والتكلفة الإجمالية وزمن العبور ±10% و±20% على كل المسارات."},
